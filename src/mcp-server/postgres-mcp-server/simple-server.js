...
console.error(`DB_MOCK_MODE env value: '${process.env.DB_MOCK_MODE}'`);
console.error(`MOCK_MODE set to: ${MOCK_MODE}`);
...
async function executeQuery(sql, params = []) {
  if (MOCK_MODE) {
    ...
    } else if (sql.toLowerCase().includes('select count(*)') && sql.toLowerCase().includes('from first_contact_ai_human')) {
      return {
        rows: [{ count: '5' }],  // Return a count of 5 for the first_contact_ai_human table
        rowCount: 1
      };
    } else if (sql.toLowerCase().includes('select') && sql.toLowerCase().includes('from first_contact_ai_human')) {
      return {
...
async function main() {
  ...
  // Continue even if the connection fails when in mock mode
  if (!connectionSuccess && !MOCK_MODE) {
    console.error('DB connection failed and not in mock mode, exiting');
    process.exit(1);
  }

  console.error(`PostgreSQL Simple MCP server running on stdio (Mock Mode: ${MOCK_MODE})`);
